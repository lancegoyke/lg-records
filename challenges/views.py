...
def challenge_filtered_list(request, slug=None):
    ...
    if slug is not None:
        ...
        context['filter'] = ChallengeFilter(request.GET, queryset=Challenge.objects.filter(
            tags__slug__in=[slug]).order_by('-date_created'))
    else:
        # use all challenges
...
class ChallengeDisplay(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['filter'] = RecordFilter(
            self.request.GET, queryset=self.get_object().records.order_by('-date_recorded'))
        challenge_records = self.get_object().records.all()
        if challenge_records.exists():
            context['top_score'] = challenge_records.order_by(
                'time_score').first().time_score
            user_records = challenge_records.filter(user=self.request.user)
            if user_records.exists():
                context['user_pr'] = user_records.order_by(
                    'time_score').first().time_score
        return context
